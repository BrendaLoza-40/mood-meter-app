...
export interface CSVDataSource {
  ...
  name: string;
  uploadedAt: string;
  parameters: string[]; // List of available parameters from the CSV
  data: Record<string, any>[]; // Parsed CSV data
  enabled: boolean;
}
...
export function getEnabledCSVSources(config: AppConfig): CSVDataSource[] {
  return config.csvDataSources.filter(csv => csv.enabled);
}
